...
async fn ui_api_contract_endpoints_return_expected_shapes() {
    ...
    assert_eq!(
        status_json["zero_fill_upload_rate_bps_5s"].as_u64(),
        Some(0)
    );
    assert_eq!(
        status_json["zero_fill_upload_rate_bps_30s"].as_u64(),
        Some(0)
    );
    ...
    assert!(downloads_json.get("reserve_calls_total").is_some());
    assert!(downloads_json.get("reserve_granted_blocks_total").is_some());
    assert!(
        downloads_json
    ...
    assert!(downloads_json.get("reserve_denied_state_total").is_some());
    assert!(
        downloads_json
            .get("reserve_empty_no_missing_total")
            .is_some()
    );
    assert_eq!(
        downloads_json
...
#[tokio::test]
async fn uploads_endpoint_lists_recent_dropped_session_reason() {
    let (kad_tx, _kad_rx) = mpsc::channel(8);
    let state = test_state(kad_tx);
    state.upload_service.note_held(
        "dropfeeddropfeeddropfeeddropfeed",
        "peer-dropped",
        0,
        511,
        Duration::from_secs(30),
