...
- [x] Post-current longrun + merge: do not mark `OP_COMPRESSEDPART` blocks received until payload is decompressed, size-validated, and persisted successfully.
...
- [x] Post-current longrun + merge: enforce outbound payload size cap in `src/i2p/sam/datagram.rs` send path.
- [ ] Post-current longrun + merge: log warning when `sam.keys` permission hardening (`chmod 600`) fails.
- [x] Post-current longrun + merge: add hostile-input regression tests for i2p module (oversized SAM lines, oversized HTTP body, malformed chunked bodies).
